...
# Set environment variables to control internal parallelization
...
def train_and_evaluate(params):
    # Generate dataset inside the function (to be repeated each time)
...
# Iterate over the parameter grid serially
results = []
for p in pg:
    print(f"Training with parameters: {p}")
    result = train_and_evaluate(p)
...
print(f"Total time taken (serial): {end_time - start_time:.2f} seconds")
