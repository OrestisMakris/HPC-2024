...
#pragma omp target data map(to: A,B,Time) map(tofrom: sum)
    {
        for (int k = 0; k < Time; ++k) {
            ...
            for (int i = 1; i < N - 1; ++i) {
                for (int j = 1; j < M - 1; ++j) {
                    B[i][j] = (A[i - 1][j] + A[i + 1][j] + A[i][j - 1] + A[i][j + 1] + 2 * A[i][j]) / 6.0f;
                }
            }
            ...
            for (int i = 1; i < N - 1; ++i) {
                for (int j = 1; j < M - 1; ++j) {
                    A[i][j] = (B[i - 1][j] + B[i + 1][j] + B[i][j - 1] + B[i][j + 1] + 2 * B[i][j]) / 6.0f;
                }
            }
        }
